...
# Added user weight input
try:
    weight = float(input("Enter your weight in kg: ").strip() or "75")
except ValueError:
    weight = 75.0
print(f"Using weight = {weight:.1f} kg")
...
# Calculate Kp, Kd based on weight
# Assume base gains are for a "standard" 75kg person
base_Kp = 200.0
base_Kd = 15.0
weight_ratio = weight / 75.0  # Scale relative to 75kg

Kp = base_Kp * weight_ratio
Kd = base_Kd * weight_ratio

print(f"\n--- Controller Gains (Scaled by Weight) ---")
print(f"Base (75kg): Kp={base_Kp:.1f}, Kd={base_Kd:.1f}")
print(f"Scaled ({weight:.1f}kg): Kp={Kp:.2f}, Kd={Kd:.2f} (Ratio: {weight_ratio:.2f}x)")
...
def simulate_cycles(k, c, f, Kp_sim, Kd_sim, theta0=theta0_mean, ncycles=8, n_per_cycle=400):
    """Simulate multiple gait cycles with consistent sampling per cycle"""
    T = 1.0 / f
    total_time = ncycles * T
    # Pass Kp_sim, Kd_sim into the parameter dictionary
    p = dict(k=k, c=c, f=f, Kp=Kp_sim, Kd=Kd_sim, theta0=theta0, m=m, g=g, L3=L3, I=I_A)
...
def rms_for_delta(dlt, ncycles=6, n_analyze_cycles=3):
    ...
    # Pass the global, weight-scaled Kp and Kd
    t_cycles, theta_cycles, thetad_cycles, theta_ref_cycles = simulate_cycles(
        k_soft, c_i, f_target, Kp_sim=Kp, Kd_sim=Kd, theta0=theta0_mean, 
...
plt.title(f"RMS tracking error vs δ (Kp={Kp:.1f}, Kd={Kd:.1f})")
...
# Pass the global, weight-scaled Kp and Kd
t_cycles, theta_cycles, thetad_cycles, theta_ref_cycles = simulate_cycles(
    k_soft, best_c, f_target, Kp_sim=Kp, Kd_sim=Kd, theta0=theta0_mean, ncycles=8, n_per_cycle=500
)
...
plt.title(f"Experimental vs Simulated (Weight={weight}kg, Kp={Kp:.1f}, Kd={Kd:.1f})")
plt.legend(); plt.grid(True, ls='--', alpha=0.6); plt.tight_layout(); plt.show()
