...
#   • + Passive-window RMS validation
#   • + Passive-window torque check
#   • + Passive-window multi-speed check
# ================================================================
...
def plot_window_highlight(t_norm, theta_mean, s, e):
    ...
    plt.title("Passive window on gait cycle")
...
def plot_top_overlays(t_win, theta_exp, k_vals, c_vals, J_grid,
    ...
    plt.show()
#-------------------------------------------------
#Sensitivity testing, this is the added part and also a part in main
#----------------------------------------------------
def passive_window_sensitivity(time_s, theta_exp,
                               I, L3, k_star, c_star, mass, g,
                               save_path=None, show_abs=False):
    """
    One-at-a-time sensitivity on the passive window:
    - Computes baseline RMSE (deg) for identified (k*, c*, I)
    - Perturbs k, c, I by +/- (20%, 20%, 10%) one-at-a-time
    - Prints a small table and plots a ΔRMSE bar chart (deg)
    """
    # --- Safety: ensure numpy arrays and strictly increasing time
    time_s = np.asarray(time_s, dtype=float)
    theta_exp = np.asarray(theta_exp, dtype=float)

    # If there are repeated times, nudge them to be strictly increasing
    dt = np.diff(time_s)
    if np.any(dt <= 0):
        eps = 1e-9
        for i in range(1, len(time_s)):
            if time_s[i] <= time_s[i-1]:
                time_s[i] = time_s[i-1] + eps

    # --- Baseline initial conditions from data
    theta0_pw  = float(theta_exp[0])
    omega0_pw  = float(np.gradient(theta_exp, time_s)[0])

    # --- Baseline simulation and RMSE
    theta_sim_baseline = simulate_theta(time_s, theta0_pw, omega0_pw,
                                        I, c_star, k_star, mass, g, L3)
    rmse_base_rad = rmse(theta_sim_baseline, theta_exp)
    rmse_base_deg = float(np.degrees(rmse_base_rad))

    # --- Define one-at-a-time perturbations
    perturbations = [
        ("k -20%", {"I": I,      "k": 0.8 * k_star, "c": c_star}),
        ("k +20%", {"I": I,      "k": 1.2 * k_star, "c": c_star}),
        ("c -20%", {"I": I,      "k": k_star,       "c": 0.8 * c_star}),
        ("c +20%", {"I": I,      "k": k_star,       "c": 1.2 * c_star}),
        ("I -10%", {"I": 0.9*I,  "k": k_star,       "c": c_star}),
        ("I +10%", {"I": 1.1*I,  "k": k_star,       "c": c_star}),
    ]

    labels, delta_rmse_deg, abs_rmse_deg = [], [], []

    # --- Evaluate RMSE for each perturbation
    for name, p in perturbations:
        th_sim = simulate_theta(time_s, theta0_pw, omega0_pw,
                                p["I"], p["c"], p["k"], mass, g, L3)
        r_deg = float(np.degrees(rmse(th_sim, theta_exp)))
        labels.append(name)
        abs_rmse_deg.append(r_deg)
        delta_rmse_deg.append(r_deg - rmse_base_deg)

    # --- Print a tiny table
    print("\nSensitivity (passive window):")
    print(f"Baseline RMSE: {rmse_base_deg:.3f} °")
    for name, r_abs, d in zip(labels, abs_rmse_deg, delta_rmse_deg):
        sign = "+" if d >= 0 else ""
        print(f"  {name:>6}  ->  RMSE = {r_abs:.3f} °  (Δ {sign}{d:.3f} °)")

    # --- Plot ΔRMSE bar chart
    plt.figure(figsize=(8, 4.2))
    x = np.arange(len(labels))
    plt.bar(x, delta_rmse_deg)
    plt.axhline(0.0, linestyle='--', linewidth=1)
    plt.xticks(x, labels, rotation=0)
    plt.ylabel("ΔRMSE (deg) vs baseline")
    plt.title("Sensitivity: ΔRMSE for k±20%, c±20%, I±10%")
    plt.tight_layout()
    if save_path:
        plt.savefig(save_path, bbox_inches="tight")
    plt.show()

    # --- Optional: absolute RMSE bar chart
    if show_abs:
        plt.figure(figsize=(8, 4.2))
        plt.bar(x, abs_rmse_deg)
        plt.axhline(5.0, linestyle=':', linewidth=1)  # example threshold
        plt.xticks(x, labels, rotation=0)
        plt.ylabel("RMSE (deg)")
        plt.title("Passive-window sensitivity: absolute RMSE")
        plt.tight_layout()
        if save_path:
            base = save_path.rsplit(".", 1)[0]
            plt.savefig(base + "_abs.pdf", bbox_inches="tight")
        plt.show()

    # return values if you want to log them
    return {
        "baseline_deg": rmse_base_deg,
        "labels": labels,
        "delta_rmse_deg": delta_rmse_deg,
        "abs_rmse_deg": abs_rmse_deg,
    }
...
# ------------------------------- #
#   NEW: window-only validation   #
# ------------------------------- #

def validate_passive_window(time_s, theta_exp, k, c,
                            theta0, omega0,
                            I, m, g, L):
    theta_sim = simulate_theta(time_s, theta0, omega0, I, c, k, m, g, L)
    err_rad = rmse(theta_sim, theta_exp)
    err_deg = np.degrees(err_rad)
    print("\nPassive-window validation:")
    print(f"  RMSE: {err_rad:.6f} rad  ({err_deg:.3f} °)")
    return theta_sim, err_rad, err_deg

def torque_validation_window(theta_win, time_s, geom, k, c, mass, g):
    """
    Compute implied torque ONLY over the passive window.
    τ = I θ¨ + c θ̇ + k θ + mgL sinθ
    """
    I = geom['I_A']; L = geom['L3']
    theta_dot = deriv(theta_win, time_s)
    theta_ddot = deriv(theta_dot, time_s)
    tau = I*theta_ddot + c*theta_dot + k*theta_win + mass*g*L*np.sin(theta_win)

    max_abs = float(np.max(np.abs(tau)))
    mean_abs = float(np.mean(np.abs(tau)))

    print("\nPassive-window torque validation:")
    print(f"  Max |τ|:   {max_abs:.2f} N·m")
    print(f"  Mean |τ|:  {mean_abs:.2f} N·m")
    return tau

def multi_speed_validation_window(t_win_norm, theta_win_exp,
                                  geom, k, c, mass, g,
                                  speeds=(2.5, 3.2, 4.0)):
    """
    For each speed, scale the SAME window in time and see how well (k,c) tracks it.
    """
    results = {}
    for spd in speeds:
        gait_s = gait_params(spd, geom['stride'])
        time_s = t_win_norm * gait_s['T']
        theta_dot0 = deriv(theta_win_exp, time_s)
        theta0 = float(theta_win_exp[0])
        omega0 = float(theta_dot0[0])
        theta_sim = simulate_theta(time_s, theta0, omega0,
                                   geom['I_A'], c, k, mass, g, geom['L3'])
        err = rmse(theta_sim, theta_win_exp)
        results[spd] = np.degrees(err)

    print("\nPassive-window multi-speed validation (RMS deg):")
    for spd, val in results.items():
        print(f"  {spd:.1f} km/h: {val:.2f} °")
    return results
...
def main():
    ...
    # 1) RMS on the passive window
    theta_sim_win, _, _ = validate_passive_window(
        time_s,
        theta_exp,
        k_star,
        c_star,
    ...
    # 2) torque ON THE PASSIVE WINDOW ONLY
    torque_validation_window(
        theta_exp,
        time_s,
        geom,
        k_star,
    ...
    # 3) multi-speed ON THE PASSIVE WINDOW ONLY
    multi_speed_validation_window(
        t_win,           # normalised times in the window
        theta_exp,       # experimental angles in the window
        geom,
        k_star,
        c_star,
        mass,
    ...
    # === Passive-window sensitivity bar chart (ΔRMSE) ===
    #This is also the added part
    passive_window_sensitivity(
        time_s=time_s,
        theta_exp=theta_exp,
        I=geom['I_A'],
        L3=geom['L3'],
        ...
        mass=mass,
        g=g,
        save_path=None,   # e.g. "fig/sensitivity_delta_rmse.pdf"
        show_abs=False    # set True to also plot absolute RMSE bars
    )
